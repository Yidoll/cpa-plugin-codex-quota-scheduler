...
type StatusAccount struct {
	...
	SubscriptionRank             *int               `json:"subscription_rank,omitempty"`
...
func ConfigFromSettings(base Config, payload SettingsPayload) (Config, error) {
	...
	cfg.SelectionStrategy = payload.SelectionStrategy
	cfg.SubscriptionOrder = cloneStringSlice(payload.SubscriptionOrder)
...
func handleImportState(store *PluginState, body []byte, now time.Time) pluginapi.ManagementResponse {
	...
	validatedConfig, err := ValidateConfig(state.Config)
	if err != nil {
		return jsonManagementResponse(http.StatusBadRequest, map[string]string{"error": err.Error()})
	}
...
func buildStatusPayload(snapshot StateSnapshot, ordered []ScheduledAccount, lifecycle *ManagementLifecycleSnapshot) StatusPayload {
	...
	for i, scheduled := range ordered {
		...
		if !account.SubscriptionExpiresAt.IsZero() {
			...
			status.SubscriptionExpiresAt = &expiresAt
		}
		if rank, known := ranks[status.PlanType]; known {
			rankCopy := rank
			status.SubscriptionRank = &rankCopy
		}
		status.StrategyKnown, status.StrategyValue = strategySortValue(view, policy)
...
<div class="setting-with-help"><label class="field"><span data-i18n="settings.selectionStrategy">选择策略</span><select id="selectionStrategy"><option value="" data-i18n="settings.strategyLegacy">Legacy（兼容旧排序）</option><option value="quota_high">quota_high</option><option value="quota_low">quota_low</option><option value="subscription_high">subscription_high</option><option value="subscription_low">subscription_low</option><option value="expiry_soon">expiry_soon</option></select></label><p class="setting-help" data-i18n="settings.selectionStrategyHelp">策略只在可用性等级和插件优先级相同时生效；未知数据排在已知数据之后。</p></div>
<div class="setting-with-help"><label class="field"><span data-i18n="settings.subscriptionOrder">订阅顺序</span><textarea id="subscriptionOrder" placeholder="free, plus, pro"></textarea></label><p class="setting-help" data-i18n="settings.subscriptionOrderHelp">按从低到高填写，以逗号或换行分隔。订阅策略必须配置且不能重复。</p></div>
...
const INLINE_TRANSLATIONS=[
  ...
  ['订阅：','Subscription: '],['策略值','Strategy value'],['瓶颈配额','Bottleneck quota'],['订阅到期','Subscription expiry'],['未知','Unknown'],['缓存时间','Cache age'],['熔断计数','Circuit count'],['失败','Failures'],['成功','Successes'],['主动重置','Reset credits'],['调度状态','Scheduler state'],[' 次',' times'],['累计','total'],['有效期','expires'],
